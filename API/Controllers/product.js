...
export const addProduct = async (req,res) =>{
    ...
    try {
        let product = await Products.create({title,description,price,category,quantity,imgSrc});
...
export const updateProductById = async (req,res) =>{
    const id = req.params.id;
    let prodcut = await Products.findByIdAndUpdate(id,req.body,{new:true})
    if(!prodcut) return res.json({message:"invalid id"})
    res.json({message:"Product has been updated",prodcut})
